...
#define IS_VERSION_2_2 (VERSION_MAJOR == 2 && VERSION_MINOR == 2)
...
// FIXME - jbradach - 2016/09/20 - this seems fucky.  If at least one element
// is true, the whole thing is true?  Review what is using this.
static gboolean
...
// FIXME - jbradach - 2016/09/21 - ...whhhyyy not memcpy?  Check usages.
...
dissect_flags(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
	...
	if (encrypted) // if encrypted, add MIC status
	{
		...
		{
			// /* MIC is bad */
            #if IS_VERSION_1_11 || IS_VERSION_2_2
